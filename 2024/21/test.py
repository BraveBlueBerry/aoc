...
@cache
def get_directions_for_directional(from_d: str, to_d: str):
    next_c = []
    arrows.reset()
    arrows.add_start(navArrows[from_d])
    if arrows.starting_position == navArrows[to_d]:
        return ['A']
    arrows.add_end(navArrows[to_d])
    paths = arrows.bfs_find_all_paths()
    for p in paths:
        arrows.visualize_path(p['path'], p['directions'])
        next_c.append(p['directions'][1:] + ['A'])

    return next_c

def get_directions_for_command(command: list):
    next_cs = []
    arrows.reset()
    ...
    for key_press in command:
        ...
        if navArrows[key_press] == arrows.starting_direction:
            ...
            for p in paths:
                arrows.visualize_path(p['path'], p['directions'])
                command_options_for_key_press.append(p['directions'][1:] + ['A'])
            arrows.add_start(navArrows[key_press])
        next_cs.append(command_options_for_key_press)
    print(next_cs)
    return next_cs
...
for code in result:
    ...
    print(robot_one)
    print(robot_two)
    print(whole_command_for_code)
    print(len(whole_command_for_code))
